<?php
class AdmManualsController extends AppController
{
    ...
    function index($id = null)
    {
        ...
        $nav = $this->CustomerNav->find('list', array(
            'conditions' => array('CustomerNav.manuals' => '1'),
        ));
    ...
    function block_index()
    {
        $this->paginate['conditions'] = array(
              'Manual.customer_organization_id' => $this->request->named['customer_organization_id'],
              'Manual.driver_manual_type_id' => $this->request->named['type_list'],
            );
    ...
    function add()
    {
        if(!empty($this->request->named['customer_organization_id'])){
            ...
            $customer_organization_id = $this->request->data['Manual']['customer_organization_id'];
        }
        
        if(!empty($this->request->named['driver_manual_type_id'])){
            $driver_manual_type_id = $this->request->named['driver_manual_type_id'];
        } else {
            $driver_manual_type_id = $this->request->data['Manual']['driver_manual_type_id'];
        }
        ...
        if($this->ActionAdd->run(array('default' => $default,
            ...
            $this->flash(
                $message,
                "/adm_manuals/index/{$customer_organization_id}/driver_manual_type_id:{$driver_manual_type_id}/",
                Configure::read('App.pause')
            );
        } else {
            // 戻るURL設定
            $back_url = '/adm_manuals/index/' . $customer_organization_id . 
    ...
    function edit($id = null)
    {
        ...
        if(empty($id) && !empty($this->request->data['Manual']['id']))
           $id = $this->request->data['Manual']['id'];
        if(!empty($this->request->data['Manual']['customer_organization_id'])) {
            $customer_organization_id = $this->request->data['Manual']['customer_organization_id'];
            $driver_manual_type_id = $this->request->data['Manual']['driver_manual_type_id'];
        } else {
            $record = $this->Manual->findById($id);
            $customer_organization_id = $record['Manual']['customer_organization_id'];
            $driver_manual_type_id = $record['Manual']['driver_manual_type_id'];
        }
        // 分類リスト取得
        $type_list = $this->DriverManualType->find('list', array(
                'conditions' => array('DriverManualType.driver_manual_id' => $this->driver_manual_id,
