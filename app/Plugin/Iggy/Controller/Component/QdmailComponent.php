...
if ( defined('CAKE_CORE_INCLUDE_PATH') || defined('CAKE')) {
	class QdmailBranch extends Component{
	}
}else{
	class QdmailBranch{
	}
...
class QdmailBase extends QdmailBranch{
	...
	var $LFC_Qmail			=  null;
	//var $is_qmail			=  null;
	var $is_qmail			=  false; // iggy changed.
	var $language			= 'ja';
...
class QdDeco{
	...
	function get( $kind ){
		if(!empty($this->request->data[$kind])){
			return $this->request->data[$kind];
		}
		if('ATTACH'===$kind){
			return array();
		}else{
	...
	function decode(){
		...
		$this->request->data['HTML'] = $receiver->bodyAutoSelect() ;
		if(false===$this->request->data['HTML']){
			$this->request->data['HTML'] = '';
		}
		...
		foreach($attach as $att){
			...
			$this->request->data['ATTACH'][]=array(
